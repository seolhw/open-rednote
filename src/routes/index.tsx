    ...
    <main className="mx-auto w-full max-w-[1160px] px-4 pb-14 pt-8 sm:pt-10">
      ...
      <section className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <article key={label} className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
            <p className="text-xs text-zinc-500">{label}</p>
            <h2 className="mt-1 text-2xl font-semibold text-zinc-900">{value}</h2>
          </article>
        ))}
      </section>
      ...
      <section className="mt-4 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="mt-4 grid gap-3">
          {[
            ...
            <article key={title} className="grid grid-cols-[2rem_1fr] gap-3 rounded-xl border border-zinc-200 p-3">
